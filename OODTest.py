...
num_workers = 30
batch_size = 64
output_base_dir = 'output_student'
...
for student in ["resnet18"]:
    for dataset in list(med_datasets.keys())[1:6]:
        data = MedMNISTDataModule(batch_size, num_workers, dataset)
        PATH = "./results/"+student+"_student_/best_model.pth"
        checkpoint = torch.load(PATH)
        state_dict = {k.replace(" ",""): v for k, v in checkpoint.items()}
        model_ = teachers_dict[student](pretrained=True)
        #model_.load_state_dict(state_dict)
        output_name = student+"_teacher_" + dataset + "_finetuning_"
        print(output_name)
        finetune(data, model_, output_name, data.num_classes)