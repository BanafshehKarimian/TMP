...
for student in ["resnet18"]:
    if True:
        dataset = "CelebA_"
        data = CelebADataModule(batch_size, num_workers, index = 0)
        PATH = "./results/"+student+"_student_MSE/best_model.pth"
        checkpoint = torch.load(PATH)
        state_dict = {k.replace(" ",""): v for k, v in checkpoint.items()}
        model_ = teachers_dict[student](pretrained=True)
        model_.load_state_dict(state_dict)
        output_name = student+"_student_" + dataset + "_finetuning_MSE"
        print(output_name)
        finetune(data, model_, output_name, 40)
