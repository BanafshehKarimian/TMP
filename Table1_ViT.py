...
from utils.teachers_dict import teachers_dict, teachers_dict_vit
...
column_name = ["Method", "Model"]#
datsets = ["CIFAR10", "DTD", "STL10", "SVHN", "FGVCAircraft", "cub"]#
for d in datsets:
...
teachers = list(teachers_dict_vit.keys())
teachers.append("resnet18")
for model in teachers:
    results = ["NoKD", model, ]
    for task in datsets:
        if not os.path.isfile('./output_teacher/' + model + '_'+ task +'_teacher_finetuning/predictions.csv'):
            results.append(0)
            continue
